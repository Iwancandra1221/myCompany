<script>
    ...
    $(document).ready(function() {
        ...
        $("#kodebarang").change(function(){   
			...
			$.post("<?php echo site_url('reportservicev2/carinamabarang'); ?>", {
        ...
        // $("#report").change(function(){
            // if ($("#report").val()!="09" && $("#report").val()!="05") {
                // alert ("Selain Report 05 dan 09 Laporan Summary Service Harian, yang lain masih dalam pengerjaan!");
            // } 
        // });
    ...
    function Browse(div){
        if(div==1){
            ...
            var merk = $('#merk').val();
            $.ajax({
	            url: "<?php echo site_url('reportservicev2/listbarangsesuaidivisi'); ?>",
                type: 'post',
                data: {merk: merk},
				dataType: 'json',
                success: function(response){ 
					...
					for(i=0;i<response.length;i++){
                        html+='<tr>';
                            html+='<th width="50px">'+response[i].KD_BRG+'</th>';
                            html+='<th width="300px">'+response[i].KD_BRG+'</th>';
                            html+='<th width="500px">'+response[i].NM_BRG+'</th>';
                            html+='<td width="150px" align="center"><a href="javascript:PilihBarang(\''+response[i].KD_BRG+'\',\''+response[i].NM_BRG+'\')" class="btn btn-dark btn-sm btnPilihBarang" data=""><span class="fa fa-edit"></span> Pilih Barang </a></td>';
                        html+='</tr>';
					}
...
<div class="container">
	...
	<?php 
        echo form_open("reportservicev2/proses_report", array("target"=>"_blank"));
	?>
