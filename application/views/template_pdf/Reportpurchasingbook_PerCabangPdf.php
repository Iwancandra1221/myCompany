...
foreach($rekapTmp as $keyKat => $valueKat){

	foreach($valueKat as $keyTmp => $valueTmp){
		$Lokasi2 = $keyTmp;
			...
			<table autosize="1" style="overflow: wrap;width:100%;">
				<thead>
					<tr>
						...
						<td colspan="2" style="font-weight:bold;">{$Lokasi2}</td>
					</tr>
					<tr style="">
						...
						<td style="border: solid 1px black;width:10%;font-size: 10px;padding: 2mm">DPP + PPN</td>
						<td style="border: solid 1px black;width:4%;font-size: 10px;padding: 2mm">LOK</td>
						<td style="border: solid 1px black;width:6%;font-size: 10px;padding: 2mm">TGL INVOICE</td>
					</tr>
				</thead>
				<tbody>
HTML;
			...
			foreach($rekap as $key => $value){
				...
				if($tmpNo_FakturP!=$value['No_FakturP']){
					...
					$col4 = $value['NPWP'];
					$col5 = $value['Nm_Supl'];
					$col14 = $value['Kd_Lokasi2'];
					$col15 = date('d/m/Y',strtotime($value['Tgl_Invoice']));
...
HTML;
				...
				if($lengthRekap - 1 == $key){
					//kondisi ini untuk menambah 1 row di akhir index
					$grandTotalDisc += $totalDisc;
					$grandTotalDpp += $totalDpp;
					$grandTotalPpn += $totalPpn;
					$grandTotalDppPpn += ($totalDpp + $totalPpn);
...
HTML;
				...
				<tr>
					...
					<td style="border-left:solid 1px black; border-right:solid 1px black;font-size: 11px;padding: 2mm;">{$col15}</td>
				</tr>
				{$rowEndTotal}
HTML;
			}
			...
			$katBrg = '';
			if($keyKat=='P') $katBrg = 'PRODUK';
			else if($keyKat == 'S') $katBrg = 'SPAREPART';
			else $katBrg = $keyKat;

			echo <<<HTML
					<tr>
						<td colspan="10" style="border:solid 1px black;font-size: 10px;padding: 2mm;text-align:right;">TOTAL {$keyRekap} - {$Lokasi2} - {$katBrg}</td>
...
<table autosize="1" style="overflow: wrap;width:100%;">
	<tr>
		<td colspan="10" style="font-size: 9px;text-align: right; width: 71%;">Grand Total</td>
		<td style="text-align: right;font-size: 7px;padding: 2mm;width: 8%">{$grandGrandTotalDpp}</td>
		<td style="text-align: right;font-size: 7px;padding: 2mm;width: 8%">{$grandGrandTotalPpn}</td>
		<td style="text-align: right;font-size: 7px;padding: 2mm;width: 8%">{$grandGrandTotalDppPpn}</td>
		<td style="font-size: 7px;padding: 2mm;width: 2.5%;"></td>
		<td style="font-size: 7px;padding: 2mm;width: 2.5%;"></td>
	</tr>
</table>
HTML;
