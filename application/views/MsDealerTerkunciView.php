...
<script>
    function btnFakturGantung(kdPlg,nmPlg,wil){
    ...
    function btnRequest(kdPlg,nmPlg,wil){
        $(".loading").show();
        var csrf_bit = $("input[name=csrf_bit]").val();
    ...
    function btnResend(ReqID){
        $(".loading").show();
          var csrf_bit = $("input[name=csrf_bit]").val();
          $.post("<?php echo site_url('MsDealer/ResendRequestUnlock/1'); ?>", {
            KodeRequest : ReqID,
            csrf_bit    : csrf_bit
          }, function(data){
            if (data.result == "sukses") {
    ...
    function btnCancelReq(ReqID,BtnID){
...
<div class="container">
  ...
  <table id="maintable" class="table table-striped table-bordered" cellspacing="0">
    <thead>
      <tr>
        <th scope="col" class="hideOnMobile">No</th>
        <th scope="col" class="hideOnMobile">Kode</th>
        <th scope="col" >Nama Pelanggan</th>
        <th scope="col" class="hideOnMobile">Wilayah</th>
        <th scope="col" class="hideOnMobile">CreditLimit</th>
        <th scope="col" ></th>        
        <th scope="col" ></th>
      </tr>
    </thead>
    <tbody>
	  ...
	  // if(count($dealers)==0){
		// echo "<tr>";
		// echo "<td align='center' colspan='7'>TIDAK ADA DATA</td>";
		// echo "</tr>";
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    //     var tdk_aktif = 0;
    //   }
    //       $("#TblFakturGantung").dataTable().fnFilter(
    //         aktif,0,
    //         tdk_aktif,0,
    //       );
    //   });
  <?php
    if(!empty($menu)){
      if($menu=='TokoTerkunciMO'){
        $url=site_url('MsDealer/ListTokoTerkunciMO');
      }else if($menu=='TokoTerkunci'){
        $url=site_url('MsDealer/ListTokoTerkunci');
      }else if($menu=='TokoTerkunciProyek'){
        $url=site_url('MsDealer/ListTokoTerkunciProyek');
      }
  ?>
      $('#maintable').dataTable( {
          "bProcessing": true,
          "bServerSide": true,
           "columnDefs": [
            {"title":"No","targets": 0,"className":"hideOnMobile"},
            {"title":"Kode","targets": 1,"className":"hideOnMobile"},
            {"title":"Nama Pelanggan","targets": 2,"className":"hideOnMobile"},
            {"title":"Wilayah","targets": 3,"className":"hideOnMobile"},
            {"title":"CreditLimit","targets": 4, "className":"hideOnMobile", "orderable": false},
            {"title":"","targets": 5, "orderable": false},
