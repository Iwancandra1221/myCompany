...
<script>
  ...
  function number_format(number, decimals, decPoint, thousandsSep){
    ...
    function getdata(){
      ...
      $("#txtPartnerTypePlg").val(sArray[3]);
      // alert(sArray[3]);
      
      var Div = $("#selDiv").val();

      if ((sArray[3]=="MODERN OUTLET" || sArray[3]=="MO CABANG") && (Div!="MO")) {
        alert("Dealer MO hanya bisa menggunakan Divisi MO. Divisi otomatis diganti ke MO!");
        $("#selDiv").val("MO");
      } else if (sArray[3]=="PROYEK" && Div!="MISHIRIN") {
        alert("Dealer PROYEK menggunakan CL MISHIRIN untuk pengetikan Produk+Sparepart. Divisi otomatis diganti ke MISHIRIN!");
        $("#selDiv").val("MISHIRIN");  
      } else if (Div=="MO" && sArray[3]!="MODERN OUTLET" && sArray[3]!="MO CABANG") {
        alert("Dealer Non MO tidak bisa menggunakan Divisi MO.\nDivisi otomatis diganti ke MISHIRIN.\nGanti manual ke SPAREPART untuk pengajuan limit sparepart!");
        $("#selDiv").val("MISHIRIN");
      }
  ...
  $(document).ready(function(){
    ...
    $("#btnRequestCL").click(function(){
      var PartnerType = $("#txtPartnerTypePlg").val();
      ...
      if (PartnerType=="MODERN OUTLET" || PartnerType=="MO CABANG" || PartnerType=="PROYEK") {
  ...
  var LoadCLGroup = function(){
    ...
    $.post("<?php echo site_url('MsDealer/GetCreditLimitGroup'); ?>", {
      ...
      if (data.result == "sukses") {
        ...
        //   $.ajax({ 
        //       type: 'GET', 
        //       url: '<?php echo site_url("MsDealer/GetConfigColor?telat='+List[i].MAKS_TELAT+'") ?>',
        //       dataType: 'json',
        //       success: function (data){ 

        //         // if(data.length>0){   
        //           STYLE = "background-color:"+data[0].ConfigValue+";";   
        //           FLAG =  data[0].ConfigType; 
        //           BRS ="";
        //           List[i].STYLE = STYLE;
        //           List[i].FLAG = FLAG;
        //           alert(List[i].KD_PLG+" "+List[i].FLAG);
        //         // }
        //       }
        //   });  
        // }
        ...
        for(var i=0;i<List.length;i++) {
          ...
          BRS ="";
          BRS+="<tr style='padding:5px;' id='tr_"+KDLOKASI+"_"+KODE+"'>";
      ...
      if (data.result == "sukses") {
        ...
        for(var i=0;i<List.length;i++) {
          ...
          if (List[i].RequestStatus=="WAITING FOR APPROVAL") {
            ACTION+= "<a href='ResendRequest/1?type=CL&id="+List[i].RequestNo+"' target='_blank'><button>Resend Request</button></a>"
...
<div class="container">
  ...
  <?php echo form_open(site_url('MsDealer/RequestCL')); ?>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control txtplg" name="txtPlg" id="txtPlg" placeholder="Nama Dealer" required>
      
      <button type="button" class="btn btn-default btncheck" style="font-size:12px; font-weight: normal;" onclick="getdata()">
    ...
    <div class="form-group">
      ...
      <input type="hidden" class="form-control" name="txtPartnerTypePlg" id="txtPartnerTypePlg">
    </div>
    <div class="form-group">
    ...
    <div class="form-group form-request-cl">
      ...
      <table id="tableGroupDealer" class="table table-stripped table-bordered" summary="Informasi Limit Group Pelanggan">
        <thead>
          <tr>
            <td width="5%"></td>
