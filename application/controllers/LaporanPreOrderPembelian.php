...
class LaporanPreOrderPembelian extends MY_Controller 
{
	public $excel_flag = 0;
	...
	public function index()
	{
		...
		$params['Module']="LAPORAN PREORDER PEMBELIAN"; 
   		$params['TrxID'] = date("YmdHis");
		$params['Description']=$_SESSION["logged_in"]["username"]." BUKA MENU LAPORAN PREORDER PEMBELIAN";
		...
		$api = 'APITES';
		//die($this->API_URL."LaporanPreOrderPembelian/CheckDivisi?api=".$api."&p_user=".urlencode($_SESSION['logged_in']['useremail']));
		set_time_limit(60);
		/*die($this->API_URL."LaporanPreOrderPembelian/GetKodePrePO?api=".$api);*/
		$result = json_decode(file_get_contents($this->API_URL."/LaporanPreOrderPembelian/GetKodePrePO?api=".$api));
		// die($this->API_URL."LaporanPreOrderPembelian/CheckUser?api=".$api."&p_user=".urlencode($_SESSION['logged_in']['useremail']));
		$check_menu = json_decode(file_get_contents($this->API_URL."/LaporanPreOrderPembelian/CheckUser?api=".$api."&p_user=".urlencode($_SESSION['logged_in']['useremail'])));
		//die((string)$check_menu);
		$url = $this->API_URL."/LaporanPreOrderPembelian/CheckDivisi?api=".$api."&p_user=".urlencode($_SESSION['logged_in']['useremail']);
		// die($url);
		$check_divisi = json_decode(file_get_contents($url));
		// die(json_encode($check_menu));
		$branches = $this->BranchModel->GetsByUser($_SESSION['logged_in']['useremail']);
		$data['title'] = 'Laporan PreOrder Pembelian | '.WEBTITLE;
		$data['menu'] = $check_menu;
		$data['divisi'] = $check_divisi;
		$data['kode_prepo'] = $result;
		$data['months'] = $this->HelperModel->GetMonths();

		$this->RenderView('LaporanPreOrderPembelianFormView',$data);
		//$this->load->view('LaporanPreOrderPembelianFormView',$data);
	}
	...
	{ 
		...
		if(isset($_POST["btnPreview"])){
			...
			$params['Module']="LAPORAN PREORDER PEMBELIAN"; 
	   		$params['TrxID'] = date("YmdHis");
			$params['Description']=$_SESSION["logged_in"]["username"]." PROSES PREVIEW LAPORAN ".$_POST["opsi"];
	   		$params['Remarks']="";
			...
			$params['Module']="LAPORAN PREORDER PEMBELIAN"; 
	   		$params['TrxID'] = date("YmdHis");
			$params['Description']=$_SESSION["logged_in"]["username"]." PROSES EXPORT EXCEL LAPORAN ".$_POST["opsi"];
	   		$params['Remarks']="";
	   		$params['RemarksDate'] = 'NULL';
	   		$this->ActivityLogModel->insert_activity($params); 
		}
		...
		if(isset($_POST['cbg']))
		{
			...
			if($this->form_validation->run())
			{
				...
				if ($_POST["opsi"]=="C01") {
					$this->Preview_DelapanPeriode("GABUNGAN", $page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"],$params);
				} else if ($_POST["opsi"]=="C02") {
					$this->Preview_DelapanPeriode("KOTA", $page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"],$params);
				} else if ($_POST["opsi"]=="C03") {
					$this->Preview_DelapanPeriode("GROUP GUDANG", $page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"],$_POST["cbox1"],$params);
				} else if ($_POST["opsi"]=="A01") {
					$this->Preview_SatuPeriodeAllKota($page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"], $params);
				} else if ($_POST["opsi"]=="A02") {
					$this->Preview_SatuPeriodeAllPT($page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"],$params);
				} else if ($_POST["opsi"]=="B01") {
					$this->Preview_JualBeliAllKota("JUAL", $page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"],$params);
				} else if ($_POST["opsi"]=="B02") {
					$this->Preview_JualBeliAllKota("BELI", $page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"],$params);
				} else if ($_POST["opsi"]=="B03") {
					$this->Preview_JualBeliAllPT("JUAL", $page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"],$params);
				} else if ($_POST["opsi"]=="B04") {
					$this->Preview_JualBeliAllPT("BELI", $page_title, $_POST["yyyy"], $_POST["mm"], $_POST["pp"], $_POST["divisi"], $_POST["cbg"], $_POST["cbox1"],$params);
				}
			}
			else
			{
	...
	public function Preview_SatuPeriodeAllKota($page_title, $p_yy, $p_mm, $p_pp, $p_divisi, $p_cbg, $p_hide_nol='N',  $params)
	{   
		$data = array();
		$api = 'APITES';
		...
		*/
		...
		if($this->excel_flag == 1){
			$sheet->setTitle('LaporanForecastBeliJual');
			$sheet->setCellValue('A1', 'LAPORAN FORECAST BELI/JUAL/STOCK PER PERIODE ALL KOTA');
			$sheet->getStyle('A1')->getFont()->setSize(20);
			$sheet->setCellValue('A2', 'Divisi : '.$p_divisi);
			$sheet->setCellValue('A3', 'Periode : '.$p_pp);
			$sheet->setCellValue('A4', 'Bln/Thn : '.$p_mm."/".$p_yy);
		}
		...
		if($this->excel_flag == 1){
			$sheet->setCellValue('A6', 'Kode Barang');
		}
		$currcol = 1;
		$currrow = 6;
		...
		{	
			...
			if($this->excel_flag == 1){
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $PrePo[$i]->Kota.' S_Awal');
				$currcol += 1;
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $PrePo[$i]->Kota.' Beli');
				$currcol += 1;
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $PrePo[$i]->Kota.' Jual');
				$currcol += 1;
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $PrePo[$i]->Kota.' S_Akhir');
				$currcol += 1;
			}
		...
		if($this->excel_flag == 1){
			$sheet->setCellValueByColumnAndRow($currcol, $currrow, 'Total Beli');
			$currcol += 1;
		...
		for ($j=0; $j<count($arrBrg);$j++)
		{
			...
			if($this->excel_flag == 1){
				$sheet->setCellValueByColumnAndRow(0, $currrow+$j, $arrBrg[$j]);
			}
			...
			for($i=0;$i<count($PrePo);$i++)
			{
				...
				if($this->excel_flag == 1){
					$sheet->setCellValueByColumnAndRow($currcol+$i, $currrow+$j, $qty_s_awal);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol+$i, $currrow+$j, $qty_t_beli);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol+$i, $currrow+$j, $qty_t_jual);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol+$i, $currrow+$j, $qty_s_akhir);
				}	
			...
			if($this->excel_flag == 1){	
				$sheet->setCellValueByColumnAndRow(count($PrePo)*4+1, $currrow+$j, $qty_total_beli);
				$currcol += 1;
				$sheet->setCellValueByColumnAndRow(count($PrePo)*4+2, $currrow+$j, $qty_total_jual);
			}	
		...
		if($this->excel_flag == 1){
			...
			$filename='LaporanForecastBeliJualStockPerPeriodeAllKota['.date('Ymd').']'; //save our workbook as this file name
	...
	public function Preview_SatuPeriodeAllPT($page_title, $p_yy, $p_mm, $p_pp, $p_divisi, $p_cbg, $p_hide_nol='N', $params)
	{
		...
		*/
		...
		if($this->excel_flag == 1){
			...
			$sheet->setTitle('LaporanForecastBeliJual');
			$sheet->setCellValue('A1', 'LAPORAN FORECAST BELI/JUAL/STOCK PER PERIODE ALL BIT/PT');
			$sheet->getStyle('A1')->getFont()->setSize(20);
			$sheet->setCellValue('A2', 'Divisi : '.$p_divisi);
			$sheet->setCellValue('A3', 'Periode : '.$p_pp);
			$sheet->setCellValue('A4', 'Bln/Thn : '.$p_mm."/".$p_yy);
		...
		{	
			...
			if($this->excel_flag == 1){
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $str_kota.' S_Awal');
				$currcol += 1;
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $str_kota.' Beli');
				$currcol += 1;
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $str_kota.' Jual');
				$currcol += 1;
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $str_kota.' S_Akhir');
				$currcol += 1;
			}
		...
		if($this->excel_flag == 1){
			...
			$filename='LaporanForecastBeliJualStockPerPeriodeAllPT['.date('Ymd').']'; //save our workbook as this file name
			$writer = new Xlsx($spreadsheet);
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header('Content-Disposition: attachment;filename="'. $filename .'.xlsx"'); 
			header('Cache-Control: max-age=0');
	...
	public function Preview_JualBeliAllKota($mode, $page_title, $p_yy, $p_mm, $p_pp, $p_divisi, $p_cbg, $p_hide_nol='N', $params)
	{
		$data = array();
		$api = 'APITES';
		$spreadsheet = new Spreadsheet();
		...
		if($this->excel_flag == 1){
			$this->excel->setActiveSheetIndex(0);
			$sheet->setTitle('LaporanForecastBeliJual');
			$sheet->setCellValue('A1', 'LAPORAN FORECAST '.$mode.' ALL CABANG GABUNGAN');
			$sheet->getStyle('A1')->getFont()->setSize(20);
			$sheet->setCellValue('A2', 'Divisi : '.$p_divisi);
			$sheet->setCellValue('A3', 'Periode : '.$p_pp);
			$sheet->setCellValue('A4', 'Bln/Thn : '.$p_mm."/".$p_yy);
		...
		{		
			...
			if($this->excel_flag == 1){
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $PrePo[$i]->Kota);
				$currcol += 1;
			}
		}
		...
		for ($j=0; $j<count($arrBrg);$j++)
		{
			...
			for($i=0;$i<count($PrePo);$i++)
			{
				...
				if($this->excel_flag == 1){
					$sheet->setCellValueByColumnAndRow($currcol+$i, $currrow+$j, $qty);
				}	
			}
			...
			if($this->excel_flag == 1){
				$sheet->setCellValueByColumnAndRow(count($PrePo)+1, $currrow+$j, $qty_total);
			}	
		...
		if($this->excel_flag == 1){
			...
			$filename='LaporanForecast'.$mode.'AllCabangAllKota['.date('Ymd').']'; //save our workbook as this file name
			$writer = new Xlsx($spreadsheet);
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header('Content-Disposition: attachment;filename="'. $filename .'.xlsx"'); 
			header('Cache-Control: max-age=0');
	...
	public function Preview_JualBeliAllPT($mode, $page_title, $p_yy, $p_mm, $p_pp, $p_divisi, $p_cbg, $p_hide_nol='N', $params)
	{
		$data = array();
		$api = 'APITES';
		$spreadsheet = new Spreadsheet();
		...
		if($this->excel_flag == 1){
			$this->excel->setActiveSheetIndex(0);
			$sheet->setTitle('LaporanForecast');
			$sheet->setCellValue('A1', 'LAPORAN FORECAST '.$mode.' ALL CABANG');
			$sheet->getStyle('A1')->getFont()->setSize(20);
			$sheet->setCellValue('A2', 'Divisi : '.$p_divisi);
			$sheet->setCellValue('A3', 'Periode : '.$p_pp);
			$sheet->setCellValue('A4', 'Bln/Thn : '.$p_mm."/".$p_yy);
		...
		{		
			...
			if($this->excel_flag == 1){
				$sheet->setCellValueByColumnAndRow($currcol, $currrow, $PrePo[$i]->Kota.' - '.$PrePo[$i]->Cbg);
				$currcol += 1;
			}
		}
		...
		if($this->excel_flag == 1){
			...
			$filename='LaporanForecast'.$mode.'AllCabangAllPT['.date('Ymd').']'; //save our workbook as this file name
			$writer = new Xlsx($spreadsheet);
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header('Content-Disposition: attachment;filename="'. $filename .'.xlsx"'); 
			header('Cache-Control: max-age=0');
	...
	public function Preview_DelapanPeriode($mode, $page_title, $p_yy, $p_mm, $p_pp, $p_divisi, $p_cbg="ALL",$p_hide_nol='N', $params)
	{
		...
		if($this->excel_flag == 1){
			...
			$this->excel->setActiveSheetIndex(0);
			$sheet->setTitle('LaporanForecast8Periode');
			$sheet->setCellValue('A1', 'LAPORAN FORECAST 8 PERIODE '.$judul);
			$sheet->getStyle('A1')->getFont()->setSize(20);
			$sheet->setCellValue('A2', 'Divisi : '.$p_divisi);
			$sheet->setCellValue('A3', 'Periode : '.$p_pp);
			$sheet->setCellValue('A4', 'Bln/Thn : '.$p_mm."/".$p_yy);
		...
		{		
		    ...
		    for($j=0;$j<count($DT);$j++) 
		    {
		    	...
		    	else
		    	{
		    		...
		    		if($this->excel_flag == 1){
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $SUBKATEGORI);
						$sheet->getStyle('A'.($currrow+$i+$j).':Z'.($currrow+$i+$j))
					    ->getFill()
	    		...
	    		if($this->excel_flag == 1){
					...
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli1);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign1);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual1);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 1)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual1,2)));
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Stock1);
					$currcol += 1;

					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli2);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign2);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual2);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 2)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual2,2)));
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Stock2);
					$currcol += 1;

					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli3);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign3);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual3);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 3)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual3,2)));
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Stock3);
					$currcol += 1;

					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli4);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign4);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual4);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 4)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual4,2)));
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Stock4);
					$currcol += 1;

					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli5);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign5);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual5);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 5)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual5,2)));
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Stock5);
					$currcol += 1;
					
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli6);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign6);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual6);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 6)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual6,2)));
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Stock6);
					$currcol += 1;
					
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli7);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign7);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual7);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 7)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual7,2)));
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Stock7);
					$currcol += 1;
					
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Beli8);
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Campaign8 );
					$currcol += 1;
					$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, $DT[$j]->Jual8);
					$currcol += 1;
					if($p_yy <= 2017 and (12-(int)$p_mm) * 2 >= 8)
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, '-');
					else
						$sheet->setCellValueByColumnAndRow($currcol, $currrow+$i+$j, ($DT[$j]->PersentaseJual1 == 0 ? '-' : round($DT[$j]->PersentaseJual8,2)));
					$currcol += 1;
		...
		if($this->excel_flag == 1){
			...
			$filename='LaporanForecast8Periode'.$judul.'['.date('Ymd').']'; //save our workbook as this file name
			$writer = new Xlsx($spreadsheet);
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header('Content-Disposition: attachment;filename="'. $filename .'.xlsx"'); 
			header('Cache-Control: max-age=0');
