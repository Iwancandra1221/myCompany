...
class MsDealerApprovalOld extends NS_Controller {
    ...
    public function ProsesRequestUnlock()
    {
    	...
    	$REQUEST = $this->MsDealerModelOld->GetRequestUnlockToko($KodeRequest);
		...
		if (date("Ymd",strtotime($REQUEST->RequestDate))!=date("Ymd")) {
			$data["EXPIRED"] = true;
	    } else {
	    	$data["EXPIRED"] = false;	    	
	    }
		...
		if ($UserPwd=="") {
			...
			if ($res->result == "SUKSES" || "SKIP"=="NOT SKIP"){
				if (isset($_POST["btnApprove"])) {
					$this->ApproveUnlock($RequestNo, $UserEmail, $EndDate, $Note, $mc);
				} else {
					$this->RejectUnlock($RequestNo, $UserEmail, $EndDate, $Note, $mc);
				}
	...
	public function doApproveUnlock()
	{
		$KodeRequest="UL/DMIC005/2203/04";
		$ApprovedBy="hkawanto@yahoo.com";
		$EndDate="2022-03-19";
		$Note="OK";
		$this->ApproveUnlock($KodeRequest, $ApprovedBy, $EndDate, $Note);
	}

    public function ApproveUnlock($KodeRequest, $ApprovedBy, $EndDate, $Note="", $mc=0)
    {
    	$data = array();
    	...
    	if ($rq!=null) {
    		if ($rq->IsCancelled==1) {
		        ...
		        if ($result["result"]=="sukses") {
		        	if ($this->MsDealerModelOld->ApproveUnlockToko($KodeRequest, $ApprovedBy, $EndDate, $Note)) {
    ...
    public function RejectUnlock($KodeRequest, $ApprovedBy, $EndDate, $Note="", $mc=0)
    {
    	...
    	if ($rq!=null) {
			if ($rq->IsCancelled==1) {
	    		...
	    		if ($this->MsDealerModelOld->RejectUnlockToko($KodeRequest, $ApprovedBy, $EndDate, $Note)) {
    				$this->NotifikasiApprovalUnlockToko($KodeRequest);
    ...
    public function NotifikasiApprovalUnlockToko($KodeRequest) 
    {
    	$rq = $this->MsDealerModelOld->GetRequestUnlockToko($KodeRequest);
		...
		$e_content = "*".$rq->AppName." ".(($rq->IsApproved==1)?"MENYETUJUI":"MENOLAK")."* PERMINTAAN UNLOCK DEALER TERKUNCI\n";
		$e_content.= "Nama Dealer : *".$rq->NmPlg."*\n";
		$e_content.= "Kode Dealer : *".$rq->KdPlg."*\n";
		$e_content.= "Wilayah : *".$rq->Wilayah."*\n";
		$e_content.= "No Ref : *".$KodeRequest."*\n";
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => API_MSG.'/sendMessageWA',
    ...
    public function EmailApprovalUnlock($KodeRequest) 
    {
		...
		if (SEND_EMAIL=="ON")
		{
			...
			$plg = $this->MsDealerModelOld->ReplaceChars($rq->NmPlg);
    ...
    public function SyncUnlockedDealer()
	{
		...
		if($keyAPI==$api) {
			set_time_limit(60);
			$requests = $this->MsDealerModelOld->GetUnlockedDealer();	
			if (count($requests)>0) 
			{
