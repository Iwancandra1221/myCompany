...
class MsDealerApproval extends NS_Controller {
	...
	public function autoApproveUnlock($KodeRequestMD5)
	{
		...
		if ($REQUEST->IsApproved==0) {
	        ...
	        $hasil = json_encode(array("result"=>"gagal", "error"=>"request sudah diapprove"));
	        header('HTTP/1.1: 200');
	        header('Status: 200');
	        header('Content-Length: '.strlen($hasil));
	        exit($hasil);
    	...
    	if ($rq!=null) {
    		...
    		if ($rq->IsCancelled==1) {
		        ...
		        if ($result["result"]=="sukses") {
		        	if ($this->MsDealerModel->ApproveUnlockToko($KodeRequest, $ApprovedBy, $EndDate, $Note)) {
    ...
    public function RejectUnlock($KodeRequest, $ApprovedBy, $EndDate, $Note="", $mc=0)
    {
    	...
    	if ($rq!=null) {
			if ($rq->IsCancelled==1) {
	    		...
	    		if ($this->MsDealerModel->RejectUnlockToko($KodeRequest, $ApprovedBy, $EndDate, $Note)) {
    				$this->NotifikasiApprovalUnlockToko($KodeRequest);
