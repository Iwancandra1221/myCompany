<?php 
	...
	class Reportservicev2 extends MY_Controller 
	{
		...
		{
        	//http://localhost:90/myCompany/Reportservicev2/index
			$data = array();
			$api = 'APITES';
			set_time_limit(60);
			//persiapkan bktAPI
			...
			//http://localhost:90/bktAPI/Reportservicev2/GetListKodeNotaService?api=APITES
			...
			//Teknisi (bktAPI)
			//http://localhost:90/bktAPI/Reportservicev2/GetListTeknisi?api=APITES
			...
			$this->RenderView('reportserviceV2view',$data);
		...
		public function proses_report()
		{
			...
			if($keyAPI==$api) {
				...
				if ($params["report"]=="01"){
					...
					$url = $AlamatBKTAPI.$this->API_BKT."/Reportservice/Report_01?api=".$api;
				} elseif ($params["report"]=="02"){
					$excel_title = "Laporan Harian Service dengan Detail Sparepart";
					$url = $AlamatBKTAPI.$this->API_BKT."/Reportservice/Report_02?api=".$api;
					...
					$url = $AlamatBKTAPI.$this->API_BKT."/Reportservice/Report_04?api=".$api;
				} elseif ($params["report"]=="05"){
					$excel_title = "Laporan Produk Masuk Service dari Pembeli/Distributor";
					$url = $AlamatBKTAPI.$this->API_BKT."/Reportservice/Report_05?api=".$api;
					...
					$url = $AlamatBKTAPI.$this->API_BKT."/Reportservice/Report_08?api=".$api;
...
HTML;
        ...
        public function proses_excel_02 ($page_title, $excel_title, $param, $DataLaporan, $output)
        {
			...
			for($i=0; $i<$jum; $i++) {
                ...
                if ( $No_Svc != $DataLaporan[$i]->No_Svc and $No_Svc != "!@#$%") {
					...
					$sheet->getStyle('A'.$currrow.':T'.$currrow)->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        ...
        public function proses_pdf_html_02 ($page_title, $excel_title, $param, $DataLaporan, $output)
        {
            ...
            $header = '<table border="0" style="width:100%; border-collapse:collapse; font-size:10px;">
                    <tr>
            ...
            $header .= '<table width="100%" style="font-weight: bold; border-collapse:collapse;">
                        </table>'
            ;
            
            $content.= "<style> th, td { font-size:8px; } </style>";			
            $content.= "<div class='div_body' style='overflow-x:padding-left:1px;'>";
            $content.= "	<table style='width:100%; border-collapse:collapse; border-top:1px solid #000'>";
            // $content.= "	<tr>";
            ...
            if ($jum!=0){
                ...
                for($i=0; $i<$jum; $i++) {
                    if ( $No_Svc != $DataLaporan[$i]->No_Svc and $No_Svc != "!@#$%" ) {
                        ...
                        if ( $Grandtotal != 0 ) {
                            ...
                            $content.= "		<tr><td colspan='6' style='".$border_bottom."'> </td>";
                            $content.= "			<td width='60px' align='right' style='border-top:1px solid #333;".$border_bottom."'>Total</td>";
                            $content.= "			<td width='60px' align='right' style='border-top:1px solid #333;".$border_bottom."'>".number_format($HargaSP)."</td>";
                            $content.= "			<td width='60px' align='right' style='border-top:1px solid #333;".$border_bottom."'>".number_format($Ongkos_Svc)."</td>";
                            $content.= "			<td width='60px' align='right' style='border-top:1px solid #333;".$border_bottom."'>".number_format($Home_Svc)."</td>";
                            $content.= "			<td width='60px' align='right' style='border-top:1px solid #333;".$border_bottom."'>".number_format($PPN)."</td>";
                            $content.= "			<td width='60px' align='right' style='border-top:1px solid #333;".$border_bottom."'>".number_format($Disc_Tambahan)."</td>";
                            $content.= "			<td width='60px' align='right' style='border-top:1px solid #333;".$border_bottom."'>".number_format($Grandtotal)."</td>";
                            $content.= "	    </tr>";
                        }
                        $HargaSP = 0;
                    ...
                    if ( $No_Svc != $DataLaporan[$i]->No_Svc ) {
                        ...
                        $content.= "		<tr><td colspan='13'></td></tr>
                                            <tr><td colspan='1'><b>".$O."&nbsp;&nbsp;&nbsp;&nbsp;".$X."</b></td>
                                                <td colspan='4'><b>".date("d-M-Y", strtotime($DataLaporan[$i]->Tgl_Svc))."&nbsp;&nbsp;&nbsp;&nbsp;".    
                                                $DataLaporan[$i]->No_Svc."&nbsp;&nbsp;&nbsp;&nbsp;". 
                                                $DataLaporan[$i]->Merk."&nbsp;&nbsp;&nbsp;&nbsp;".
                                                $DataLaporan[$i]->Kd_Brg."</b></td>".
                                                "<td colspan='3'><b>PENGADUAN</b>:".$DataLaporan[$i]->Kerusakan."</td>".
                                                "<td colspan='5'><b>PERBAIKAN</b>:".$DataLaporan[$i]->Perbaikan."".
                                    "       </td></tr>
                        ";    
                    ...
                    if ( trim($DataLaporan[$i]->No_Faktur) != "" ) {
						...
						if ( $No_Svc != $DataLaporan[$i]->No_Svc ) {
							...
							$content.= "<td></td>";
							$content.= "<td></td>";
							$content.= "<td align='right'><b>Qty</b></td>";
							$content.= "<td align='right'><b>Harga</b></td>";
							$content.= "<td align='right'><b>Sub Total</b></td>";
							$content.= "<td align='right'><b>Ongkos</b></td>";
							$content.= "<td align='right'><b>Home</b></td>";
							$content.= "<td align='right'><b>PPN</b></td>";
							$content.= "<td align='right'><b>Disc</b></td>";
							$content.= "<td align='right'><b>GrandTotal</b></td>";
							$content.= "</tr>";    
                ...
                if ( $Grandtotal != 0 ) {
                    $content.= "		<tr style='border-bottom:1px solid #000'><td colspan='6'> </td>";
