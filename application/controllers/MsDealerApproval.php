...
class MsDealerApproval extends NS_Controller {
		...
		if ($REQUEST!=null) {
			if ($REQUEST->IsApproved==0 && $isAutoApproved) {
		    	...
		    	if ($REQUEST->ExpiryDate == null) {
		    		$data["EXPIRED"] = false;
		    	} else if (date("Ymd",strtotime($REQUEST->ExpiryDate)) < date("Ymd")) {
					$data["EXPIRED"] = true;
			    } else {
			    	$data["EXPIRED"] = false;	    	
			    }
	...
	public function ProcessRequestV2()
	{
		...
		if ($GetRequest["result"]=="sukses") {
	        ...
	        if ($mc==0) {
		        $this->SetTemplate('template/notemplate');
		        $this->load->view("MsDealerRequestViewV2", $data);
		    } else {
		    	$this->SetTemplate('template/template');
	...
	public function ApproveRejectRequestV2(){
		...
		if ($UserEmail=="-" || $UserEmail=="") {
			redirect("MsDealerApproval/ProcessRequestV2?type=".$tipe."&id=".urlencode($RequestNo)."&alert=".urlencode("USER BELUM DIPILIH")."&mc=".$mc);
		} else if ($UserPwd!="") {
			//$zen = API_ZEN."/ZenAPI/CheckLogin?user=".urlencode($UserEmail)."&pwd=".urlencode(md5($UserPwd));
			//$res = json_decode(file_get_contents($zen, true));
	...
	public function UpdateIsLockedDebug()
	{
		$result = $this->MsDealerModel->UpdateIsLocked("DMIB097", "DMI");
    ...
    public function UpdateIsLocked()
	{
		...
		$token = '';
		$err = '';
		$url = base_url().'Sync/GetTokenAPIv2?branch='.$branch;
		// die($url);
		...
		// die($token);
		$PHOENIXURL = '';
		$url = base_url().'Sync/GetPhoenixURL';
		// die($url);
