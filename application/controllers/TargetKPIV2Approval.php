...
class TargetKPIV2Approval extends NS_Controller 
{
	...
	{
		$URL = $this->API_URL."/Targetkpiv2/AmbilTargetKPI?req=".urlencode($noRequest)."&app=".urlencode($approvedBy);
		// echo($URL); die;
		...
		if($GetRequest["result"]=="SUCCESS") {
			...
			for($i=0; $i<count($details); $i++) {
				...
				$detailhd.="    <td rowspan='".$k."'>".$dt["NAMA"]."<br>[".$dt["USERID"]."]</td>";                
	...
	public function ApproveReject()
	{
		...
		if(ISSET($data['karyawan'])){
			...
			CURLOPT_URL => $this->API_URL."/Targetkpiv2/ApproveTargetKPI",
			...
			if ($result["result"]=="SUCCESS") {
				// $deadline_emailed = [];
				// if(ISSET($result['data']['data']->DEADLINE_EMAILED)){
					// $deadline_emailed = $result['data']['data']->DEADLINE_EMAILED;
				// }
				$resultEdit = $this->Targetkpiv2model->UpdateApproval($data['no_request'], $data['app_by']); //$deadline_emailed
				...
				// foreach($data['karyawan'] as $kodeTarget){
					...
					$resultEdit = $this->Targetkpiv2model->editTblApproval($wApproval,$dApproval);
					*/
					
					/*
					$getNextPriority = $this->Targetkpiv2model->getNextPriority($data);
					if($getNextPriority){
						
						$post['api'] = 'APITES';
						$post['norequestkpi'] = $getNextPriority->RequestNo;
						$post['kpicategory'] = $getNextPriority->AddInfo1Value;
						$post['user_name'] = $getNextPriority->RequestBy;
						$post['user_email'] = $getNextPriority->RequestByEmail;
						
						$post['job'] = $getNextPriority->ApprovedBy;
						$post['nama'] = $getNextPriority->ApprovedByName;
						$post['email'] = $getNextPriority->ApprovedByEmail;
						+
						$post['branch_id'] = $getNextPriority->AddInfo5Value;
						$post['periode'] = $getNextPriority->AddInfo6Value;
						$post['week'] = $getNextPriority->AddInfo12Value;
						// echo json_encode($post); die;
					
						$URL = $this->API_URL."/Targetkpiv2/sendrequest2";
						$curl = curl_init();
						curl_setopt_array($curl, array(
						CURLOPT_URL => $URL,
						CURLOPT_RETURNTRANSFER => true,
						...
						if($httpcode!=200){
							...
							if($result['result']=='success'){
								// foreach($result['data'] as $res){
								   $dApproval = array(
										'IsEmailed' => $result['is_emailed'],
										'EmailedDate' => date('Y-m-d H:i:s'),
									);
									$wApproval = array(
										'RequestNo' => $data['no_request'],
										'ApprovalStatus' => 'UNPROCESSED',
										'ApprovedByEmail' => str_replace('"','',$getNextPriority->ApprovedByEmail)
									);
									$resultEdit = $this->Targetkpiv2model->editTblApproval($wApproval,$dApproval);
								// }
							}
		...
		//REJECT
			...
			$curl = curl_init();
			curl_setopt_array($curl, array(
			CURLOPT_URL => $this->API_URL."/Targetkpiv2/RejectTargetKPI",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_TIMEOUT => 60,
			CURLOPT_POST => 1,
			CURLOPT_POSTFIELDS => http_build_query($data),
		...
		$this->ViewRequestTargetKPI($data["no_request"], $data["app_by"], $data["total_week"], $msg);
	}
	...
	{
		$URL = $this->API_URL."/Achievementkpiv2/AmbilAchievementKPI?req=".urlencode($noRequest)."&app=".urldecode($approvedBy);
		// echo($URL);die;
		$GetRequest = json_decode(file_get_contents($URL), true);
		// echo(json_encode($GetRequest));die;
		...
		if($GetRequest["result"]=="SUCCESS") {
			...
			$detailhd.= "   <th width='2%'>History/Status</th>";
			$detailhd.= "</tr>";
			...
			for($i=0; $i<count($details); $i++) {
				...
				$detailhd.="    <td rowspan='".$k."' class='target'>".$dt["NAMA"]."<br>[".$dt["USERID"]."]";   
	...
	public function ApproveRejectAchievement() 
	{
		...
		if(ISSET($data['karyawan'])){
			$curl = curl_init();
			curl_setopt_array($curl, array(
			CURLOPT_URL => $this->API_URL."/Achievementkpiv2/ApproveAchievementKPI",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_TIMEOUT => 60,
			CURLOPT_POST => 1,
			CURLOPT_POSTFIELDS => http_build_query($data),
			...
			$result = json_decode($result, true);
			if ($result["result"]=="SUCCESS") {
				$resultEdit = $this->Targetkpiv2model->UpdateApproval($data['no_request'], $_SESSION['logged_in']['useremail']); //$deadline_emailed
		...
		//REJECT
			...
			$url = $this->API_URL."/Targetkpiv2/RejectAchievementKPI";
