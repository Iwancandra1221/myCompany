<?php //if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class PreOrderPembelianApproval extends NS_Controller 
	{
		public $cc = "";
			...
			if(isset($prepo->header)){
				...
				*{
				...
				if ($header->Ket_Status=="WAITING FOR APPROVAL" || $header->Ket_Status=="WAITING FOR APPROVAL KACAB" || 1==0) {
					echo "
					<form action='".site_url('PreOrderPembelianApproval/ApprovalNewAndUpdateTblApproval')."' method='POST'>
		...
		public function ApprovalNewAndUpdateTblApproval() {
			...
			if(ISSET($data['kd_brg'])){
				...
				if ($result->pesan=='sukses'){
					...
					$params['ApprovedByEmail'] = $approvedby;
					$params['ApprovalNote'] = '';
					if($acc_by!=''){
						$params['Priority'] = 1;
					} else {
		...
		public function ApprovalNewAndUpdateTblApprovalOld() {
			...
			$approvedbyid = ""; 
			$approvedby = (($acc_by!="")? $confirmed_by : $acc_by);
			$user = $this->UserModel->getUserDataByEmail($approvedby);
			if ($user != null) {
				$approvedbyid = $user->USERID;
			} else {
			...
			//REJECT
				...
				if ($result->pesan=='sukses') {
					...
					$params['ApprovedBy'] = $approvedbyid;
					$params['ApprovalNote'] = urlencode($rejectnote);
					$this->approval->doaction('reject',$params);
		...
		{   
			// $data = '{               "RequestTo": "BM" ,      "RequestNo": "PPOP/BDG/2307/012" ,      "RequestBy": "-" ,       "RequestByName": "RICKY UTAMA",      "RequestByEmail": "candrawenz1221@gmail.com" ,      "ApprovedBy": "" ,      "ApprovedByName": "" ,      "ApprovedByEmail": "" ,      "AddInfo2Value": "RINNAI",      "AddInfo6Value": "AUG 2023 PERIODE 1" ,      "AddInfo9Value": "BANDUNG"      }';
			...
			if (date("d")<"15") {
				$ExpiryDate = date("Y-m-15");
			}

			if($trx->RequestTo=='KACAB'){
				...
				$post["amount"] = 0; 
				$post["branchID"] = "";
				$x = $this->approvalmodel->doaction('insert', $post);
				echo json_encode($x);
			}
			else{
				// die("BM");
