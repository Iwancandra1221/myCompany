...
class MsDealer extends MY_Controller {
	...
	public function RequestBukaLockToko()
	{
		...
		if (isset($post['KDPLG'])){
			...
			if ($CreateRequest["result"]=="SUCCESSFUL") {
				...
				if ($sendRequest==true) {
					$SendRequest = $this->MsDealerModel->EmailRequestUnlockToko($KodeRequest);
    ...
    public function ResendRequestUnlock()
    {
		...
		if(isset($post['KodeRequest'])){
	    	$Reemail = $this->MsDealerModel->EmailRequestUnlockToko($post["KodeRequest"]);
					...
					if ($params["KenaikanCL"]>100000000) {
						...
						if ($params["IsBass"]==1) {
							if ($params["Wilayah"]=="OUTLIER" || ($params["BranchDB"]=="JKT" && $params["NamaDB"]=="JAKARTA")) {
								...
								$this->addRecipients_Mgr("SERVICE");
								$REQUESTID = $this->MsDealerModel->RequestCL($params, $email_content, $this->approvers, $user, $REQUESTID["requestid"]);
							}
						} else {
